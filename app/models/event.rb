class Event < ActiveRecord::Base
  ...
  scope :published, -> { where(published: true).order('start_date desc') }
  scope :season_one, -> { where("id <= ?", 6).where(published: true).order('start_date desc') }
  scope :season_two, -> { where("id > ?", 6).where("id <= ?", 15).where(published: true).order('start_date desc') }
  scope :season_three, -> { where("id > ?", 15).where(published: true).order('start_date desc') }
